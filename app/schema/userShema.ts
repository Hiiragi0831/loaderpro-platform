...
export const userSchema = toTypedSchema(
  z.object({
    ...
    position: z
      .string({ required_error: "Введите должность" })
    ...
    // Адреса
    actualAddress: z
      .string({ required_error: "Введите фактический адрес" })
      .min(1, "Введите фактический адрес"),
    legalAddress: z
      .string({ required_error: "Введите юридический адрес" })
      .min(1, "Введите юридический адрес"),
    postalAddress: z
      .string({ required_error: "Введите почтовый адрес" })
      .min(1, "Введите почтовый адрес"),
