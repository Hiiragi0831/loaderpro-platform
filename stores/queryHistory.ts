...
export const useQueryHistoryStore = defineStore('queryHistory', {
  ...
  actions: {
    ...
    // Предзагрузка данных для следующих страниц
    async preloadNextPage(currentParams: QueryHistoryParams) {
      const nextPageParams = {
        ...currentParams,
        page: currentParams.page + 1,
      }

      const cacheKey = this.generateCacheKey(nextPageParams)
      const cachedEntry = this.cache.get(cacheKey)

      if (!cachedEntry || !this.isCacheValid(cachedEntry)) {
        // Предзагружаем в фоне без показа лоадера
        this.fetchQueryHistory(nextPageParams).catch(() => {
